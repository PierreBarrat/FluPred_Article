...
%DIF DEL /dev/fd/63    Mon Dec 28 13:50:23 2020
%DIF ADD article.tex   Mon Dec 28 13:50:14 2020
...
In both cases, the probability of fixation seems to be mainly dictated by the current frequency $f$ at which the mutation is observed}\DIFdelend \DIFaddbegin \DIFadd{, we find that new amino acid mutations fix moderately but consistently more often then their current frequency, while synonymous mutations conform with the neutral expectation.
In all cases, frequency is a strong predictor of fixation}\DIFaddend .

This dynamics is in apparent contradiction with evidence that influenza surface proteins \DIFaddbegin \DIFadd{in A/H3N2 }\DIFaddend are under strong selective pressure to evade human immune responses \citep{bhatt_genomic_2011}.
If strong selection was present, we would expect rising amino acid mutations to fix at a distinctively higher frequency than the one at which they are \DIFdelbegin \DIFdel{measured}\DIFdelend \DIFaddbegin \DIFadd{observed}\DIFaddend .
...
Next, we searched for features of mutations \DIFaddbegin \DIFadd{in A/H3N2 }\DIFaddend that allow prediction of fixation beyond frequency\DIFdelbegin \DIFdel{by dividing frequencies into categories that deviate from the diagonal in panels }\textbf{\DIFdel{C}} %DIFAUXCMD
...
\subsection{\DIFadd{Correcting for nested trajectories}} %DIF >  (fold)
...
}$$ \DIFadd{d(X_1,X_2) = \frac{1}{\vert T_1\cap T_2\vert}\sum_{t\in T_1\cap T_2} \frac{\vert S_1(t)\cap S_2(t)\vert}{\vert S_1(t)\cup S_2(t)\vert}, }$$
