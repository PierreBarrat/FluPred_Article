...
%DIF DEL /dev/fd/63    Fri Dec  4 17:51:25 2020
%DIF ADD article.tex   Fri Dec  4 17:48:26 2020
...
\DIFaddbegin \DIFadd{Since segments in influenza do not recombine, trajectories defined as above are sometimes heavily correlated with each other. 
...
\DIFadd{Here, we assess the ability of four fitness scores computed in \mbox{%DIFAUXCMD
\citep{huddleston_integrating_2020} }\hspace{0pt}%DIFAUXCMD
to predict fixation: mutational load and epitope antigenic novelty introduced in \mbox{%DIFAUXCMD
...
and the previously mentioned Local Branching Index (LBI) \mbox{%DIFAUXCMD
...
\begin{figure}
	...
	\caption{\DIFaddFL{Ability of fitness scores based on }\textbf{\DIFaddFL{A}} \DIFaddFL{LBI, }\textbf{\DIFaddFL{B}} \DIFaddFL{HI titer, }\textbf{\DIFaddFL{C}} \DIFaddFL{mutational load and }\textbf{\DIFaddFL{D}} \DIFaddFL{epitope antigenic novelty to predict fixation of mutation frequency trajectories in the HA gene of A/H3N2. Similar to figure \ref{fig:pfix_stratified} of the main text.}}
	\label{fig:pfix_v_freq_indivjohnsmodels}
\end{figure}

\DIFadd{Next, we use two fitness scores based on linear combinations of the following quantities: mutational load + LBI and mutational load + HI antigenic novelty. 
These two combinations were found in \mbox{%DIFAUXCMD
\citep{huddleston_integrating_2020} }\hspace{0pt}%DIFAUXCMD
to be the best predictors of future population. 
