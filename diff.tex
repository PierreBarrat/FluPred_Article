...
%DIF DEL /dev/fd/63    Wed Dec 23 09:58:45 2020
%DIF ADD article.tex   Wed Dec 23 09:57:17 2020
...
\DIFdelbegin \DIFdel{In the rest of this work, we will }\DIFdelend \DIFaddbegin \DIFadd{Since genomic segments of influenza do not recombine, concurrent trajectories of mutations on the same segment will be correlated.
%DIF > Indeed, the trajectories corresponding to two mutations that partly appear on the same background will tend to show a similar behavior.
...
\DIFadd{We }\DIFaddend focus on frequency trajectories that are starting at a zero (low) frequency, \emph{i.e.} $f(t=0)=0$.
...
\DIFdelbegin \DIFdel{If novel variants arise by selection, we expect to see a stronger signal of selection after conditioning on these novel variants. 
}\DIFdelend %DIF > If novel variants arise by selection, we expect to see a stronger signal of selection after conditioning on these novel variants.
In classic models of population genetics, strongly advantageous variants undergo rapid selective \emph{sweeps}, i.e., \DIFdelbegin \DIFdel{the rapid rise and fixation}\DIFdelend \DIFaddbegin \DIFadd{they rise rapidly and fix}\DIFaddend .
...
\DIFaddbegin \DIFadd{Interference between multiple such advantages mutations will perturb the trajectories \mbox{%DIFAUXCMD
\citep{illingworth_components_2012} }\hspace{0pt}%DIFAUXCMD
but we still expect that they rise in frequency on average.
}\DIFaddend 
...
\DIFaddbegin \DIFadd{This use of the frequency propagator to quantify the strength of selection was also proposed in \mbox{%DIFAUXCMD
\citep{strelkowa_clonal_2012}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend 
...
For }\DIFaddend A/H1N1pdm\DIFdelbegin \DIFdel{show only small deviations from it}\DIFdelend \DIFaddbegin \DIFadd{, we find that new mutations fix moderatly but consistently more often then their current frequency}\DIFaddend .
In both cases, \DIFdelbegin \DIFdel{the probability of fixation seems to be mainly dictated by the current frequency $f$ at which the mutation is observed}\DIFdelend \DIFaddbegin \DIFadd{frequency is a strong predictor of fixation}\DIFaddend .
...
Next, we searched for features of mutations \DIFaddbegin \DIFadd{in A/H3N2 }\DIFaddend that allow prediction of fixation beyond frequency by dividing frequencies into categories that deviate from the diagonal in panels \textbf{C} and \textbf{D} of figure~\ref{fig:fix_loss_active}.
...
\DIFaddbegin \DIFadd{Indeed, \mbox{%DIFAUXCMD
\citet{illingworth_components_2012} }\hspace{0pt}%DIFAUXCMD
have shown that background fitness and interference have substantial effects on frequyency trajectories.
}

\DIFadd{Nevertheless, we expect that signatures of intrinsic fitness benefits remain.
}\DIFaddend We design a simple model of population evolution based on the \texttt{ffpopsim} simulation software to test this hypothesis~\cite{10.1093/bioinformatics/bts633}.
...
\DIFaddbegin \pierre{We could change point (i) below to only mention A/H3N2. }
\DIFaddend This lead us to conclude that (i) influenza virus evolution is qualitatively different from models of rapidly adapting population (despite clear evidence for frequent positive selection), and (ii) \DIFdelbegin \DIFdel{previous }\DIFdelend \DIFaddbegin \DIFadd{many }\DIFaddend methods to predict influenza evolution \DIFdelbegin \DIFdel{work primarily because they pick }\DIFdelend \DIFaddbegin \DIFadd{are unable to predict future frequency dynamics even though they work well for predicting the composition of the future population.
...
%DIF >  work primarily because they pick strains that represent the future well, not because they predict future dynamics.
...
This suggests that selective sweeps are not representative of typical trajectories. \DIFaddbegin \pierre{We could cite the Illingworth paper here. They do show that as well, in a different way.}
\DIFaddend 
...
\DIFadd{In a companion article \mbox{%DIFAUXCMD
\citep{huddleston_integrating_2020}}\hspace{0pt}%DIFAUXCMD
, authors developed several models to predict the future HA population for A/H3N2 influenza.
