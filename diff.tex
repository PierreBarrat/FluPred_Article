...
%DIF DEL /home/pierrebc/T̩l̩chargements/tmpFlu/article.tex   Tue Nov 17 13:09:52 2020
%DIF ADD article.tex                                           Fri Jul 31 17:10:44 2020
%DIF < \documentclass[reprint,amsmath,amssymb,superscriptaddress,showpacs,rmp]{revtex4-1}
%DIF -------
\documentclass[reprint,amsmath,amssymb,superscriptaddress,showpacs,pre]{revtex4-1} %DIF > 
%DIF -------
...
\begin{abstract}
	...
	\DIFdelbegin \DIFdel{Simulations }\DIFdelend \DIFaddbegin \DIFadd{These results are in contrast to simulations }\DIFaddend of models of adapting populations, \DIFdelbegin \DIFdel{in contrast, show }\DIFdelend \DIFaddbegin \DIFadd{where we find the }\DIFaddend clear signals of predictability. 
...
\end{abstract}
\DIFdelbegin %DIFDELCMD < \date{\today}
%DIFDELCMD < %%%
...
\DIFdelbegin \DIFdel{The trajectory of a mutation will depend on its own effect on fitness, the contribution of the genetic background on the same segment, and the effect of the remaining seven segments. 
Here, we investigate properties of broad categories of mutations effectively averaging over different genetic backgrounds to isolate the effects intrinsic to the mutation. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{First, we ask whether we can }\DIFdelend \DIFaddbegin \DIFadd{There are different ways to make this question more specific. First, one can try to }\DIFaddend quantitatively predict the frequency of $X$ at future times $f(t)$. In other words, having observed a mutation at frequencies $(f_1, f_2,\ldots,f_n)$ at dates $(t_1,t_2,\ldots,t_n)$, what can we say about its frequency at future dates $(t_{n+1}, t_{n+2},\ldots)$?
...
\DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{In this work, we }\DIFaddend use amino-acid sequences of the HA and NA genes of A/H3N2 (since the year 2000) and A/H1N1pdm (since the year 2009) influenza available in GISAID \citep{shu2017gisaid} (see supplementary materials for an acknowledgment of all data contributors). 
...
\DIFdelbegin \DIFdel{The sweep of a mutation can be due to its own fitness effect, to the genetic background or to the effect of the seven other segments. 
...
}\DIFdelend If such sweeps are common in the evolution of HA and NA, the restriction to trajectories that start at low frequency\DIFdelbegin \DIFdel{should thus }\DIFdelend \DIFaddbegin \DIFadd{, should }\DIFaddend enrich for mutations that are positively selected and on their way to fixation.
...
\label{sub:data}
	...
	Strain names and accession numbers are given as tables in two supplementary files. Outliers strains listed at \DIFdelbegin %DIFDELCMD < \url{https://github.com/PierreBarrat/FluPredictibility/src/config} %%%
\DIFdelend \DIFaddbegin \url{https://github.com/nextstrain/seasonal-flu/tree/081be2f487182252627418491be36cd01d645954/config} \DIFaddend were removed. \\
	The code used to generate the figures presented here is available at \url{https://github.com/PierreBarrat/FluPredictibility}. 
%DIF <  https://github.com/nextstrain/seasonal-flu/tree/081be2f487182252627418491be36cd01d645954/config
% subsection data (end)
...
\DIFdel{This work was funded in part by NIAID (R01AI127893-01) and the SNF (310030\_188547).
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \bibliographystyle{unsrtnat}
...
\label{sub:supplementary_figures}
	...
	\begin{figure*}
		...
		\caption{Equivalent to \DIFdelbeginFL \DIFdelFL{panels }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{panel }\DIFaddendFL \textbf{B} \DIFdelbeginFL \DIFdelFL{and }\textbf{\DIFdelFL{C}} %DIFAUXCMD
\DIFdelendFL of figure \ref{fig:trajectories} of the main text for A/H1N1pdm influenza. $89$ trajectories are shown and participate to the mean (thick black line). }
