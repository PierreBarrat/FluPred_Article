...
%DIF DEL /dev/fd/63    Wed Jan 13 09:21:54 2021
%DIF ADD article.tex   Wed Jan 13 09:21:27 2021
...
\label{sec:introduction}
	...
	Other approaches by \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citet{Steinbrueck12123, neher_prediction_2016} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{Steinbrueck12123, neher_prediction_2016,huddleston_integrating_2020} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend predict fitness by using  hemagglutination inhibition (HI) data to determine possible antigenic drift of clades in the genealogy of the HA protein.
	...
	\DIFdelbegin \DIFdel{In this work, we use HA and NA sequences of A/H3N2 and A/H1N1pdm influenza from year 2000 to 2019 to perform a retrospective analysis of }\DIFdelend \DIFaddbegin \DIFadd{A mutation with a fitness benefit should have a tendency to fix, while detrimental mutations fix less often.
	Fixation and loss of a mutation is, however, not only determined by its intrinsic fitness benefit, but also by the fitness of the background it arose on and the fitness of competing variants \mbox{%DIFAUXCMD
\citep{strelkowa_clonal_2012}}\hspace{0pt}%DIFAUXCMD
.
	This degree to which these different components affect influenza evolution has been investigated by \mbox{%DIFAUXCMD
\citet{illingworth_components_2012}}\hspace{0pt}%DIFAUXCMD
, who showed that intrinsic selection, background fitness, and later interference have comparable effects on fixation.
	...
	Here, we investigate the }\DIFaddend frequency trajectories of amino acid mutations \DIFaddbegin \DIFadd{and the degree to which fixation or loss are predictable}\DIFaddend .
	...
	To our surprise, we find that the predictability of these trajectories is very limited: \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{for A/H3N2, the }\DIFaddend probability that a mutation fixes differs little from its current frequency, as would be expected if fixation happened purely by chance.
	This observation holds for many different categories of mutations, including mutations at epitope positions.
	This weak predictability is not attributable solely to clonal interference and genetic linkage, as simulation of models including even strong interference \DIFdelbegin \DIFdel{retain }\DIFdelend \DIFaddbegin \DIFadd{retains }\DIFaddend clear signatures of predictability.
...
Here, we investigate properties of broad categories of mutations effectively averaging over different genetic backgrounds to isolate the effects intrinsic to \DIFdelbegin \DIFdel{the mutation }\DIFdelend \DIFaddbegin \DIFadd{novel mutation in this category}\DIFaddend .
...
%DIFDELCMD < %%%
%DIF < Frequency trajectories allow a simple formulation of the above-stated questions. The short term prediction amounts to predicting the curve $f(t)$ starting from a given date $t_0$. On the other hand, the long term prediction amounts to predicting how long a trajectory will remain active, and whether it will fix or disappear when it finishes. 
\DIFdel{In the rest of this work, we will }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend focus on frequency trajectories that are starting at a zero (low) frequency, \emph{i.e.} $f(t=0)=0$.
...
\DIFdelbegin \DIFdel{Such distinction in novel and ancestral variants is necessary to meaningfully interrogate predictability.
...
In classic models of population genetics, strongly advantageous variants undergo rapid selective }\emph{\DIFdel{sweeps}}%DIFAUXCMD
\DIFdel{, i.e., the rapid rise and fixation. 
The sweep of a mutation can be due to its own fitness effect, to the genetic background or to }\DIFdelend \DIFaddbegin 

\DIFadd{Since genomic segments of influenza do not recombine, concurrent trajectories of mutations on the same segment will be correlated.
In the extreme case were the two mutations always appear in the same strains, their two trajectories will be identical.
This ``nesting'' of frequency trajectories may introduce biases when performing statistical analysis.
In section \ref{sub:correcting_for_nested_trajectories} of the SM, we investigate }\DIFaddend the effect of \DIFaddbegin \DIFadd{such nested trajectories by clustering them based on the strains that compose them.
While the number of trajectories that contribute to }\DIFaddend the \DIFdelbegin \DIFdel{seven other segments.
...
If such sweeps are common in the evolution of HA and NA, the restriction to trajectories that start at low frequency should thus enrich for mutations that are positively selected and on their way to fixation}\DIFdelend \DIFaddbegin \DIFadd{analysis decreases with more aggressive clustering, it does not significantly change the results presented below}\DIFaddend .

%DIF > Such distinction in novel and ancestral variants is necessary to meaningfully interrogate predictability.
%DIF > Each rising trajectory of a new mutation implies the existence of another decreasing one at the same position, since frequencies of all amino acids at a given position must sum to one.
%DIF > Since influenza viruses are evading human immunity, we expect novel variants to be enriched for beneficial mutations.
\DIFaddbegin 

%DIF > In classic models of population genetics, strongly advantageous mutations undergo rapid selective \emph{sweeps}, i.e., they rise rapidly and fix.
%DIF > In rapidly adapting population like influenza viruses, however, the sweep of a beneficial mutation is affected by the fitness of the genetic background it arose on and interference by other competing mutations \citep{strelkowa_clonal_2012}.
%DIF > By considering the average behavior of variants that are rising in frequency, we effectively average over backgrounds, and obtain the beneficial on average.
%DIF > If such sweeps are common in the evolution of HA and NA, the restriction to trajectories that start at low frequency should thus enrich for mutations that are positively selected and on their way to fixation.
%DIF > Interference between multiple such advantages mutations will perturb the trajectories \citep{illingworth_components_2012} but we still expect that they rise in frequency on average.

\DIFaddend \subsection*{Predicting future frequencies}
...
We consider the idealized case sketched in panel \textbf{A} of figure~\ref{fig:trajectories}: given the trajectory of a \emph{new} mutation, \emph{i.e.} that started at a frequency of 0, and that we observe at frequency $f_0$ at time $t_0$, what is the probability \DIFdelbegin \DIFdel{$P_{\Delta t}(f)$ }\DIFdelend \DIFaddbegin \DIFadd{$P_{\Delta t}(f\vert f_0)$ }\DIFaddend of observing it at a value $f$ at time $t_0 + \Delta t$?
\DIFaddbegin \DIFadd{Such frequency propagators were originally proposed by \mbox{%DIFAUXCMD
\citep{strelkowa_clonal_2012} }\hspace{0pt}%DIFAUXCMD
to quantify the strength of selection in influenza virus evolution.
}\DIFaddend 
...
For $f_0=0.3$, we found $120$ such trajectories in the case of A/H3N2 influenza, represented on the panel \textbf{\DIFdelbegin \DIFdel{B}\DIFdelend \DIFaddbegin \DIFadd{C}\DIFaddend } of figure~\ref{fig:trajectories}, where time is shifted such that $t_0 = 0$. The same analysis was performed for A/H1N1pdm, with the $89$ found trajectories displayed in figure \sref{fig:trajectories_h1n1}.
...
The results for different $\Delta t$ are shown in figure~\ref{fig:trajectories}\DIFdelbegin \DIFdel{C}\DIFdelend \DIFaddbegin \DIFadd{B}\DIFaddend .
...
The thick black line in Figure~\ref{fig:trajectories}\DIFdelbegin \DIFdel{B }\DIFdelend \DIFaddbegin \DIFadd{C }\DIFaddend shows the average frequency of all trajectories.
This average makes a sharp turn at $t=0$ and is essentially flat for $t>0$ in the case of A/H3N2, and slightly increasing for A/H1N1pdm (\DIFdelbegin \DIFdel{see supplement}\DIFdelend \DIFaddbegin \DIFadd{figure \sref{fig:trajectories_h1n1}}\DIFaddend ).
...
\DIFdelbegin \DIFdel{While this observation does not rule out that signatures exist that predict future frequency dynamics}\DIFdelend \DIFaddbegin \DIFadd{In contrast, the same analysis for A/H1N1pdm (Supplementary figure \sref{fig:trajectories_h1n1}) reveals a consistent upward trend of novel frequency trajectories.
In A/H3N2, however}\DIFaddend , past dynamics alone is \DIFdelbegin \DIFdel{weakly informative}\DIFdelend \DIFaddbegin \DIFadd{uninformative of future dynamics}\DIFaddend .

\begin{figure*}
	\centering
	\includegraphics[width=0.9\textwidth]{./Figures/Panel1.png}
	\caption{\textbf{A}: Sketch of the idea behind the short term prediction of frequency trajectories. Given a mutation that we have seen increasing in frequency and that we ``catch'' at frequency $f_0$ at time $t_0$, what can we say about the distribution of future frequencies $P_{\Delta t}(f\vert\,f_0)$? \textbf{B}: \DIFaddbeginFL \DIFaddFL{Distribution of future frequencies $P_{\Delta t}(f\vert\,f_0)$ for the trajectories shown in panel }\textbf{\DIFaddFL{C}} \DIFaddFL{and for specific values of $\Delta t$. }\textbf{\DIFaddFL{C}}\DIFaddFL{: }\DIFaddendFL All frequency trajectories of amino acid mutations in the A/H3N2 HA and NA genes that were absent in the past, are seen around $f_0=30\%$ frequency at time $t_0=0$, and are based on more than 10 sequences at each time point. Red curves represent mutations that will ultimately fix, blue the ones that will be lost, and black the ones for which we do not know the final status. Dashed horizontal lines (blue and red) represent loss and fixation thresholds. The thick black line is the average of all trajectories, counting those that fix (resp. disappear) as being at frequency 1 (resp. 0). Figure \sref{fig:trajectories_h3n2_varf0} shows equivalent figures for other values of \DIFdelbeginFL \DIFdelFL{$f0$. }\textbf{\DIFdelFL{C}}%DIFAUXCMD
\DIFdelFL{: Distribution of future frequencies $P_{\Delta t}(f\vert\,f_0)$ for the trajectories shown in panel }\textbf{\DIFdelFL{B}} %DIFAUXCMD
\DIFdelFL{and for specific values of $\Delta t$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_0$}\DIFaddendFL .}
	\label{fig:trajectories}
\end{figure*}

\subsection*{Prediction of fixation or loss}
Instead of predicting future frequency, \DIFdelbegin \DIFdel{let's consider the long-term goal of predicting }\DIFdelend \DIFaddbegin \DIFadd{we then considered }\DIFaddend the probability that a mutation fixes in the population.
...
Panels \textbf{A} and \textbf{B} of figure~\ref{fig:fix_loss_active} shows the fraction of frequency trajectories in HA and NA that either have fixed, were lost or remained active as a function of the time elapsed since they were first seen above 25\% frequency \DIFaddbegin \DIFadd{for A/H3N2 and A/H1N1pdm, respectively}\DIFaddend .
...
The \DIFaddbegin \DIFadd{fixation }\DIFaddend probability of a mutation \DIFdelbegin \DIFdel{fixing given that it is seen }\DIFdelend at frequency $f$, $P_{fix}(f)$, is then estimated by the fraction of those trajectories which terminate at a frequency larger than $95\%$, \emph{i.e.} our fixation threshold.
Panels \textbf{C} and \textbf{D} of figure~\ref{fig:fix_loss_active} show $P_{fix}(f)$ as a function of $f$ for NA and HA.
\DIFdelbegin \DIFdel{For both proteins, the probability of fixation of a new mutation at frequency $f$ is close to $f$ itself, that is $P_{fix}(f)\simeq f$. 
This result is exactly what is expected in a population evolving in }\DIFdelend \DIFaddbegin 

\DIFadd{In }\DIFaddend the \emph{absence} of selection\DIFaddbegin \DIFadd{, we expect $P_{fix}(f)$ to equal $f$}\DIFaddend .
...
Panel \textbf{C} of figure~\ref{fig:fix_loss_active} indicates that mutations in the surface proteins of A/H3N2 influenza are in good agreement with the neutral expectation\DIFdelbegin \DIFdel{, while those in }\DIFdelend \DIFaddbegin \DIFadd{.
For }\DIFaddend A/H1N1pdm\DIFdelbegin \DIFdel{show only small deviations from it. 
...
%DIFDELCMD < %%%
\DIFdel{This dynamics is in apparent contradiction with evidence that influenza surface proteins are under strong selective pressure to evade human immune responses \mbox{%DIFAUXCMD
\citep{bhatt_genomic_2011}}\hspace{0pt}%DIFAUXCMD
.
If strong selection was present, we would expect rising }\DIFdelend \DIFaddbegin \DIFadd{, we find that new }\DIFaddend amino acid mutations \DIFdelbegin \DIFdel{to fix at a distinctively higher frequencythan the one at which they are measured.
In an extreme case where most trajectories would be clean sweeps, $P_{fix}(f)$ should be close to $1$ for all but very small values of $f$}\DIFdelend \DIFaddbegin \DIFadd{fix moderately but consistently more often then their current frequency, while synonymous mutations conform with the neutral expectation.
In all cases, frequency is a strong predictor of fixation}\DIFaddend .

%DIF > This dynamics is in apparent contradiction with evidence that influenza surface proteins in A/H3N2 are under strong selective pressure to evade human immune responses \citep{bhatt_genomic_2011}.
%DIF > If strong selection was present, we would expect rising amino acid mutations to fix at a distinctively higher frequency than the one at which they are observed.
%DIF > In an extreme case where most trajectories would be clean sweeps, $P_{fix}(f)$ should be close to $1$ for all but very small values of $f$.
\DIFaddbegin 

\DIFaddend Next, we searched for features of mutations \DIFaddbegin \DIFadd{in A/H3N2 }\DIFaddend that allow prediction of fixation beyond frequency\DIFdelbegin \DIFdel{by dividing frequencies into categories that deviate from the diagonal in panels }\textbf{\DIFdel{C}} %DIFAUXCMD
\DIFdel{and }\textbf{\DIFdel{D}} %DIFAUXCMD
\DIFdel{of figure~\ref{fig:fix_loss_active}}\DIFdelend .
...
Both groups have \DIFdelbegin \DIFdel{identical probability }\DIFdelend \DIFaddbegin \DIFadd{indistinguishable probabilities }\DIFaddend of fixation, suggesting that LBI carries very little information on the probability of fixation of a mutation.

Next, we focused on previously reported antigenic sites in the A/H3N2 HA protein, referred to as \emph{epitope} positions.
Mutations at these \DIFdelbegin \DIFdel{position }\DIFdelend \DIFaddbegin \DIFadd{positions }\DIFaddend might mediate immune escape and \DIFdelbegin \DIFdel{are therefore likely }\DIFdelend \DIFaddbegin \DIFadd{might be expected to be }\DIFaddend under strong selection and show sweep-like behavior.
...
Only mutations at the 7 \DIFdelbegin \DIFdel{epitope }\DIFdelend \DIFaddbegin \DIFadd{pivotal }\DIFaddend sites reported in~\cite{Koel976} have higher chances of fixation than expected by chance.
No clear difference is found for the lists by \citet{luksza_predictive_2014, wolf_long_2006}, while positions from \citet{Shih6283} \DIFdelbegin \DIFdel{show lower chances of fixation}\DIFdelend \DIFaddbegin \DIFadd{fix less often}\DIFaddend .
...
\DIFdelbegin \DIFdel{Two ways of categorising mutations, however, suggest some power to predict }\DIFdelend \DIFaddbegin \DIFadd{In supplementary figures \sref{fig:pfix_v_freq_indivjohnsmodels} and \sref{fig:pfix_v_freq_combijohnsmodels}, we repeat this analysis for the best predictors from our recent comparison of predictive models for influenza \mbox{%DIFAUXCMD
\citep{huddleston_integrating_2020}}\hspace{0pt}%DIFAUXCMD
.
...
\DIFadd{In addition to mutational load, we find that parallel evolution is predictive of }\DIFaddend fixation.
...
We \DIFdelbegin \DIFdel{conduct }\DIFdelend \DIFaddbegin \DIFadd{conducted }\DIFaddend the same analysis \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend A/H1N1pdm\DIFdelbegin \DIFdel{influenza}\DIFdelend , with results shown in figure \sref{fig:pfix_stratified_h1n1}.
...
Panel \textbf{D} differs between figures~\ref{fig:pfix_stratified} and \sref{fig:pfix_stratified_h1n1}, with convergent evolution giving less information on fixation \DIFdelbegin \DIFdel{in the latter case}\DIFdelend \DIFaddbegin \DIFadd{for A/H1N1pdm}\DIFaddend .
However, \DIFdelbegin \DIFdel{this could be due to the shorter time period over which }\DIFdelend \DIFaddbegin \DIFadd{the number of recurring mutations for }\DIFaddend A/H1N1pdm \DIFdelbegin \DIFdel{evolved, resulting in a shorter tree and less possibilities of convergent evolution. 
Indeed, error bars for mutations appearing multiple times }\DIFdelend in \textbf{D} of figure~\sref{fig:pfix_stratified_h1n1} \DIFdelbegin \DIFdel{are relatively large, indicating a lower amount of trajectories}\DIFdelend \DIFaddbegin \DIFadd{is small and confidence intervals large}\DIFaddend . \\
...
}\DIFdelend %DIF > Frequency of mutations might thus be amplified through different sampling biases, making the connection between geographic spread, seasonality and mutation frequency non-trivial to measure.
...
One possible explanation for the weakly predictable \DIFdelbegin \DIFdel{behaviour }\DIFdelend \DIFaddbegin \DIFadd{behavior }\DIFaddend of mutations (beyond their current frequency) might be tight genetic linkage inside each segment and strong competition between different adaptive mutations \citep{neher_genetic_2011,strelkowa_clonal_2012}.
\DIFdelbegin \DIFdel{We design }\DIFdelend \DIFaddbegin \DIFadd{Indeed, \mbox{%DIFAUXCMD
\citet{illingworth_components_2012} }\hspace{0pt}%DIFAUXCMD
have shown that background fitness and interference have substantial effects on frequency trajectories.
}

\DIFadd{To investigate the degree to which interference can explain the observations, we simulate }\DIFaddend a simple model of \DIFdelbegin \DIFdel{population evolution based on the }\DIFdelend \DIFaddbegin \DIFadd{influenza evolution using }\DIFaddend \texttt{ffpopsim}\DIFdelbegin \DIFdel{simulation software to test this hypothesis}\DIFdelend ~\cite{10.1093/bioinformatics/bts633}.
The model represents a population of binary genomes of length $L=200$ evolving in a fitness landscape that changes through time. \DIFaddbegin \DIFadd{A similar setting was used in \mbox{%DIFAUXCMD
\citep{strelkowa_clonal_2012}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend 
...
Every $\Delta t$ \DIFdelbegin \DIFdel{generation, we now }\DIFdelend \DIFaddbegin \DIFadd{generations, we }\DIFaddend introduce ``antibodies'' that target a specific sub-sequence of length $l=5$, noted $(x^{ab}_{i_1}, \ldots, x^{ab}_{i_l})$.
The positions $(i_1\ldots i_l)$ are chosen at random, while the targeted sub-sequence is the dominant state at each position.
Genomes that include the \emph{exact} sub-sequence targeted by the antibody suffer a strong fitness penalty. However, a single mutation away from that sub-sequence removes this penalty completely\DIFdelbegin \DIFdel{, resulting in a fitness landscape with very strong epistasis}\DIFdelend .
...
\DIFdelbegin \DIFdel{For all models, this curve deviates significantly from the diagonal.
This is most evident for the case of a }\DIFdelend \DIFaddbegin \DIFadd{Increasing interference reduces $P_{fix}(f)$ but even for very strong interference $P_{fix}(f)$ remains significantly larger than $f$.
}

\DIFadd{For a fairly stable }\DIFaddend simple additive fitness landscape \DIFdelbegin \DIFdel{that changes rarely }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend $\Delta t=1000$\DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{) }\DIFaddend rising mutations almost always fix\DIFdelbegin \DIFdel{in the population}\DIFdelend , with $P_{fix}(f)\simeq 1$ for any $f$ larger than a few percent.
...
\DIFdelbegin \DIFdel{In other regimes, with smaller }\DIFdelend \DIFaddbegin \DIFadd{Smaller }\DIFaddend $\Delta t$ or \DIFdelbegin \DIFdel{with }\DIFdelend strong epistatic competition \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{reduce }\DIFaddend $P_{fix}(f)$\DIFdelbegin \DIFdel{is reduced and closer to the diagonal}\DIFdelend .
...
\DIFaddbegin \DIFadd{Next, we cluster trajectories of simulated populations based on the intrinsic fitness effect of corresponding mutations and in the case of a fast changing fitness landscape ($\Delta t=10$).
...
\DIFaddend These models are not meant to be accurate models of influenza viruses evolution.
But figure~\sref{fig:simulations} does show \DIFdelbegin \DIFdel{is }\DIFdelend that the patterns observed in influenza virus evolution are only reproduced by models of adapting populations when pushing clonal competition to extreme values.
We conclude that the \DIFaddbegin \DIFadd{quasi-neutral }\DIFaddend pattern in figure~\ref{fig:fix_loss_active} may not be a straightforward manifestation of genetic linkage and clonal interference, but that some more intricate interplay of epidemiology, seasonality, human immunity and chance gives rise to the weakly predictable yet strongly selected evolutionary dynamics of IAVs.
...
Panel \textbf{B} of figure~\ref{fig:LBI_consensus} shows these distances, scaled with respect to an average strain (details in caption). It clearly shows that the top-LBI strain and the consensus sequence are indeed quite similar: out of 48 time bins, only \DIFdelbegin \DIFdel{once }\DIFdelend \DIFaddbegin \DIFadd{twice }\DIFaddend is the sequence of the top-LBI strain farther away from the consensus than the average sequence is. Moreover, the sequence of the top-LBI strain \emph{exactly} matches the consensus in 19 cases.
...
However, we could find \DIFdelbegin \DIFdel{only limited }\DIFdelend \DIFaddbegin \DIFadd{little }\DIFaddend evidence for such persistent behavior in the past 19 years of \DIFdelbegin \DIFdel{IAV }\DIFdelend \DIFaddbegin \DIFadd{A/H3N2 }\DIFaddend evolution.
This lead us to conclude that (i) \DIFaddbegin \DIFadd{A/H3N2 }\DIFaddend influenza virus evolution is qualitatively different from models of rapidly adapting \DIFdelbegin \DIFdel{population }\DIFdelend \DIFaddbegin \DIFadd{populations }\DIFaddend (despite clear evidence for frequent positive selection), and (ii) \DIFdelbegin \DIFdel{previous }\DIFdelend \DIFaddbegin \DIFadd{many }\DIFaddend methods to predict influenza evolution \DIFdelbegin \DIFdel{work primarily because they pick }\DIFdelend \DIFaddbegin \DIFadd{are unable to predict future frequency dynamics even though they work well for predicting the composition of the future population.
This is illustrated by the case of LBI, which works primarily because it picks }\DIFaddend strains that represent the future well \DIFdelbegin \DIFdel{, not because they predict }\DIFdelend \DIFaddbegin \DIFadd{and not because it is predictive of }\DIFaddend future dynamics.
\DIFaddbegin \DIFadd{A/H1N1pdm, in contrast, shows stronger signatures of predictability.
}\DIFaddend 
...
This suggests that selective sweeps are not representative of typical trajectories \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{illingworth_components_2012}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
\DIFdelbegin \DIFdel{Despite the lack of predictability of mutation frequency trajectories, influenza surface proteins show strong signatures of selection \mbox{%DIFAUXCMD
\citep{bhatt_genomic_2011,strelkowa_clonal_2012}}\hspace{0pt}%DIFAUXCMD
.
}\DIFdelend 
...
In fact, \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{huddleston_integrating_2020} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{huddleston_integrating_2020} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend reports that the rate at which the frequency of a strain changes is often a poor predictor -- consistent with our observations here.
...
At the same time, influenza virus \DIFaddbegin \DIFadd{surface proteins show strong signatures of selection \mbox{%DIFAUXCMD
\citep{bhatt_genomic_2011,strelkowa_clonal_2012}}\hspace{0pt}%DIFAUXCMD
.
Their }\DIFaddend phylogenies show clear deviations from those expected from the neutral Kingman coalescent, similar to those expected under Bolthausen-Sznitman coalescent (BSC) processes that are generated by traveling wave models of rapid evolution \citep{neher_genealogies_2013,desai_genetic_2013}.
...
A recent \DIFdelbegin \DIFdel{preprint }\DIFdelend \DIFaddbegin \DIFadd{study }\DIFaddend proposed that influenza virus evolution is primarily limited by an asynchrony between population level selection and generation of new variants within infected hosts \citep{morris_asynchrony_2020}.
...
Furthermore, selection might cause the rapid rise of a novel variant to macroscopic frequencies (observable in a global sample) but its benefit rapidly ``expires'' because competing variants catch up and/or it mediates immune escape only to a small fraction of the population \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{lee_mapping_2019}}\hspace{0pt}%DIFAUXCMD
.
Immunity to A/H1N1pdm might be less variable among humans than to A/H3N2 since A/H1N1pdm has been circulating for only 11 years.
Different age-groups therefore got exposed to more similar A/H1N1pdm variants than their initial exposure to A/H3N2}\DIFaddend .
...
\label{sub:data}
	...
	\DIFdelbegin \DIFdel{Strain namesand accession numbers}\DIFdelend \DIFaddbegin \DIFadd{Acknowledgement tables with strain names, accession numbers, and originating and submitting labs }\DIFaddend are given as \DIFdelbegin \DIFdel{tables in }\DIFdelend two supplementary files. \DIFdelbegin \DIFdel{Outliers }\DIFdelend \DIFaddbegin \DIFadd{Outlier }\DIFaddend strains listed at \DIFdelbegin %DIFDELCMD < \url{https://github.com/PierreBarrat/FluPredictibility/src/config} %%%
\DIFdelend \DIFaddbegin \url{https://github.com/PierreBarrat/FluPredictibility/tree/master/src/config} \DIFaddend were removed. \\
...
\label{sub:measuring_the_geographical_spread_of_a_mutation}
	...
	Region used are the ones defined in \DIFdelbegin \DIFdel{the }\DIFdelend \texttt{Nextstrain\DIFaddbegin \DIFadd{/flu}\DIFaddend }\DIFdelbegin \DIFdel{tool}\DIFdelend ~\cite{10.1093/bioinformatics/bty407}. Those are {North America}, {South America}, {Europe}, {China}, {Oceania}, {Southeast Asia}, {Japan \& Korea}, {South Asia}, {West Asia}, and {Africa}.
...
\DIFaddbegin \subsection{\DIFadd{Correcting for nested trajectories}} %DIF >  (fold)
\label{sub:correcting_for_nested_trajectories}

\DIFadd{The analysis of the main text computes probabilities of fixation assuming that all trajectories are independent.
...
We then conduct the same analysis as in the main text on a set of }\emph{\DIFadd{effectively independent}} \DIFadd{trajectories constructed by taking one trajectory from each cluster.
}

\DIFadd{In order to perform clustering, we define a distance between trajectories.
...
\DIFadd{We define the distance $d(X_1,X_2)$ between these two trajectories based on the average similarity of the strains $S_1$ and $S_2$ that compose them:
}$$ \DIFadd{d(X_1,X_2) = 1 - \frac{1}{\vert T_1\cap T_2\vert}\sum_{t\in T_1\cap T_2} \frac{\vert S_1(t)\cap S_2(t)\vert}{\vert S_1(t)\cup S_2(t)\vert}, }$$
\DIFadd{where $T_1\cap T_2$ is the time interval where both trajectories are active, and $\vert\cdot\vert$ denotes the number of elements of a set.
...
This leads to the two following properties of $d$:
	...
	\item \DIFadd{if $d(X_1,X_2)=0$, then $X_1$ and $X_2$ represent the same frequency trajectory. The mutations $x_1$ and $x_2$ that they correpond to always appear on the same strains and are totally linked.
	}\item \DIFadd{if $d(X_1,X_2)=1$, then $X_1$ and $X_2$ can be considered completely independent. This can be the case if the two trajectories do not occur at the same dates, *i.e.* $\vert T_1\cap T_2\vert=0$, or if their respective mutations are never present on the same genomes.
}\end{itemize}

\DIFadd{we attempt to reduce the potential statistical bias due to the nesting of trajectories by grouping them based on the above defined distance.
...
}$$ \DIFadd{d(X_i,X_j) \leq d^\star \Rightarrow \exists k: X_i,X_j\in C_k }$$
\DIFadd{and
}$$ \DIFadd{X_i\in C_k \Rightarrow \exists j\in C_k: d(X_i,X_j) \leq d^\star. }$$
\DIFadd{These condition imply that the clusters formed are the minimal ones that guarantee that any two trajectories closer than the threshold distance $d^\star$ belong to the same cluster.
The number of clusters $n$ depends on the chosen value for $d^\star$.
}

\DIFadd{We compute clusters for different values of $d^\star$ for the case of the HA gene in A/H3N2.
...
\begin{figure}
	...
	\caption{\textbf{\DIFaddFL{Top}}\DIFaddFL{: Left-axis: number $n$ of clusters as a function of the threshold distance $d^\star$. The total number of trajectories is shown as a flat orange line. Right-axis: average size of clusters as a function of $d^\star$. }\textbf{\DIFaddFL{Bottom}}\DIFaddFL{: Examples of clusters for four values of $d^\star$. The four clusters displayed are the ones to which the trajectory of mutation HA1:33R belongs. }}
	\label{fig:cluster_examples}
\end{figure}

\DIFadd{Since the choice of $d^\star$ is arbitrary and since no particular value can be chosen based on the number of clusters $n(d^\star)$, we decide to test our clustering strategy for five values, namely $d^\star\in\{0, 0.05, 0.1, 0.2, 0.49\}$.
...
\DIFadd{Once clustering is performed, we re-conduct the analysis of the main text on a set of effective trajectories.
...
\begin{figure}
	...
	\caption{\textbf{\DIFaddFL{Left}}\DIFaddFL{: Fixation probability of trajectories as a function of probability for four values of $d^\star$. The reference curve is the same as in panel A of figure \ref{fig:fix_loss_active} for the HA gene. For readability, error bars are only displayed for the reference curve. }\textbf{\DIFaddFL{Bottom}}\DIFaddFL{: Number of trajectories in each frequency bin corresponding to the left panel.}}
	\label{fig:pfix_v_freq_clusters}
\end{figure}

\DIFadd{Overall, this analysis leads us to think that even though mutations in influenza may be nested, considering trajectories as independent does not result in strong statistical biases.
...
%DIF >  subsection correcting_for_nested_mutations (end)

\subsection{\DIFadd{Ability of fitness models to predict fixation}} %DIF >  (fold)
\label{sub:ability_of_fitness_models_to_predict_fixation}

\DIFadd{\mbox{%DIFAUXCMD
\citet{huddleston_integrating_2020} }\hspace{0pt}%DIFAUXCMD
developed several models to predict the future HA population for A/H3N2 influenza.
...
\DIFadd{We first assess the ability of the four most performing fitness models based on individual scores in \mbox{%DIFAUXCMD
\citep{huddleston_integrating_2020} }\hspace{0pt}%DIFAUXCMD
to predict fixation: mutational load \mbox{%DIFAUXCMD
\citep{luksza_predictive_2014}}\hspace{0pt}%DIFAUXCMD
, hemagglutination inhibition (HI) antigenic novelty \mbox{%DIFAUXCMD
\citep{neher_prediction_2016}}\hspace{0pt}%DIFAUXCMD
, a ``delta frequency''score based on the recent increase in frequency of clades, and the previously mentioned Local Branching Index (LBI) \mbox{%DIFAUXCMD
\citep{neher_predicting_2014}}\hspace{0pt}%DIFAUXCMD
.
...
\DIFadd{The best performing individual score (and second overall) in \mbox{%DIFAUXCMD
\citep{huddleston_integrating_2020}}\hspace{0pt}%DIFAUXCMD
, LBI, does not provide any information about fixation (figure \sref{fig:pfix_v_freq_indivjohnsmodels}}\textbf{\DIFadd{A}}\DIFadd{).
This is consistent with the result found in figure \ref{fig:pfix_stratified} of the main text.
In panels }\textbf{\DIFadd{B}} \DIFadd{and }\textbf{\DIFadd{D}}\DIFadd{, we observe the same inability to predict fixation for the fitness models based on HI titer and delta frequency, which were respectively the second and third best individual scores (fifth and sixth overall) in \mbox{%DIFAUXCMD
\citep{huddleston_integrating_2020}}\hspace{0pt}%DIFAUXCMD
.
However, the score based on mutational load (panel }\textbf{\DIFadd{C}}\DIFadd{) does show a more significant predicting power, with higher fitness mutations having a $\sim 25$\% higher chance of fixing than lower fitness ones for three frequency bins in a row.
...
\begin{figure}
	...
	\caption{\DIFaddFL{Ability of fitness scores based on }\textbf{\DIFaddFL{A}} \DIFaddFL{LBI, }\textbf{\DIFaddFL{B}} \DIFaddFL{HI titer, }\textbf{\DIFaddFL{C}} \DIFaddFL{mutational load and }\textbf{\DIFaddFL{D}} \DIFaddFL{delta frequency, to predict fixation of mutation frequency trajectories in the HA gene of A/H3N2. Similar to figure \ref{fig:pfix_stratified} of the main text.}}
	\label{fig:pfix_v_freq_indivjohnsmodels}
\end{figure}

\DIFadd{Next, we use two fitness models based on linear combinations of the following quantities: mutational load + LBI and mutational load + HI antigenic novelty.
These composite models were the first and third best performing ones in \mbox{%DIFAUXCMD
\citep{huddleston_integrating_2020}}\hspace{0pt}%DIFAUXCMD
.
...
\DIFadd{Both models provide some information about fixation, with the mutational load + HI model having more success for frequencies around $0.5$.
...
\begin{figure}
	...
	\caption{\DIFaddFL{Ability of fitness scores based on two combinations of the four fitness scores mentioned above: }\textbf{\DIFaddFL{A}} \DIFaddFL{mutational load + LBI; }\textbf{\DIFaddFL{B}} \DIFaddFL{mutational load + HI titer.}}
	\label{fig:pfix_v_freq_combijohnsmodels}
\end{figure}

\DIFadd{Combining models in \mbox{%DIFAUXCMD
\citep{huddleston_integrating_2020} }\hspace{0pt}%DIFAUXCMD
typically achieved a greater performance than using single models.
...
\DIFadd{It is important to note that the prediction targets in \mbox{%DIFAUXCMD
\citep{huddleston_integrating_2020} }\hspace{0pt}%DIFAUXCMD
and in the present article are not identical.
...
%DIF >  subsection ability_of_fitness_models_to_predict_fixation (end)


\DIFaddend \subsection{Biases in frequency estimations} % (fold)
...
\label{sub:supplementary_figures}
	...
	\begin{figure}
		...
		\caption{Fixation probability as a function of frequency for the simulations discussed in the main text. \DIFdelbeginFL \textbf{\DIFdelFL{Top:}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Left:}} \DIFaddendFL Simulation without antibodies. The three colored curves reflect different rate of change for the fitness landscape. Visual inspection of the frequency trajectories indicates a typical sweep time of $\sim400$ generations. \DIFdelbeginFL \textbf{\DIFdelFL{Bottom:}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Right:}} \DIFaddendFL Simulation with antibodies. The different colored curves indicate the rate at which antibodies are introduced. }
	...
	\begin{figure}
		...
		\caption{Fixation probability as a function of frequency for the simulations discussed in the main text, with trajectories stratified according to real fitness values. ``High'' and ``low'' fitness classes are defined with respect to the median value. \DIFdelbeginFL \textbf{\DIFdelFL{Top:}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Left:}} \DIFaddendFL Simulation \DIFdelbeginFL \DIFdelFL{without antibodies and }\DIFdelendFL with \DIFdelbeginFL \DIFdelFL{changes to the fitness }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{a purely additive }\DIFaddendFL landscape \DIFaddbeginFL \DIFaddFL{that changes }\DIFaddendFL every \DIFdelbeginFL \DIFdelFL{$dt=10$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\Delta t=10$ }\DIFaddendFL generations. \DIFdelbeginFL \textbf{\DIFdelFL{Bottom:}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Right:}} \DIFaddendFL Simulation with antibodies, with a new antibody every \DIFdelbeginFL \DIFdelFL{$dt=10$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\Delta t=10$ }\DIFaddendFL generations. }
