...
\label{sec:introduction}
	...
	In this work, we use A/A/H3N2 HA and NA sequences from year 2000 to 2019 to perform a retrospective analysis of frequency trajectories of amino acid mutations. 
...
The main underlying question asked in this work is the following: given a mutation $X$ in the genome of A/A/H3N2 influenza that we observe at a frequency $f$ in the population at a given date, what can we say about the future of $X$?  \\
...
In this work, we use amino-acid sequences of the HA and NA genes of A/A/H3N2 influenza since the year 2000 available in GISAID \citep{shu2017gisaid} (see supplementary materials for an acknowledgment of all data contributors). 
This amounts to $44\,976$ HA and $36\,300$ NA sequences, with a minimum of 100 per year. These sequences are binned in non-overlapping intervals of one month. Each single-month time bin and the sequences that it contains represent a (noisy) snapshot of the A/A/H3N2 population at a given date. The number of sequences per time bin varies strongly both with year and according to the season, with earlier time bins containing around 10 sequences while more recent bins contain several hundreds (see figures~\sref{fig:nseq_per_year} and \sref{fig:nseq_per_month} in SM for details).
...
To answer this question retrospectively, we use all frequency trajectories extracted from A/A/H3N2 HA sequences that satisfy these conditions for a given $f_0$.  The number of trajectories is limited and the frequency estimates themselves are based on a finite sample and are hence imprecise. 
...
This time scale of 2-3 years is consistent with the typical coalescence time observed in phylogenetic trees of A/A/H3N2 influenza \citep{rambaut_genomic_2008,yan_phylodynamic_2019}. 
...
Panel \textbf{C} of figure~\ref{fig:fix_loss_active} indicates that mutations in the surface proteins of A/A/H3N2 influenza are in good agreement with the neutral expectation. \discuss{Add smth for A/H1N1}

\discuss{This quasi-neutral dynamics is in apparent contradiction with evidence that influenza surface proteins are under strong selective pressure to evade human immune responses \citep{bhatt_genomic_2011}.
...
\discuss{We conduct the same analysis on A/H1N1 influenza, with results visible in figure \sref{fig:pfix_stratified_h1n1}. 
...
% A natural way to explain these observations would be to refer to the seasonal characteristics of A/A/H3N2. 
% In temperate regions, the number of A/A/H3N2 infections drastically increases during winter, leading to an at first rapidly growing and then declining population of viruses. 
...
\discuss{The results shown in figures~\ref{fig:fix_loss_active} and \ref{fig:pfix_stratified} are difficult to reconcile with the notion that seasonal influenza virus evolution is driven by rapid directed positive selection.} 
...
Each time bin is considered as a snapshot of the A/A/H3N2 influenza population and we will refer to sequences in time bin $t$ as the population of the \emph{present}. 
...
However, we could find only limited evidence for such persistent behavior in the past 19 years of A/A/H3N2 evolution. \pierre{But we do find evidence for A/H1N1, not of a large magnitude.}
...
Along these lines, it is possible that the A/A/H3N2 population readily responds once population level selection is high enough by giving rise to essentially equivalent variants.
