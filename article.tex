...
\label{sec:introduction}
	...
	Other approaches by \citet{Steinbrueck12123, neher_prediction_2016,huddleston_integrating_2020} predict fitness by using  hemagglutination inhibition (HI) data to determine possible antigenic drift of clades in the genealogy of the HA protein.
...
Here, we investigate properties of broad categories of mutations effectively averaging over different genetic backgrounds to isolate the effects intrinsic to novel mutations in this category.
...
%Since influenza viruses are evading human immunity, we expect novel variants to be enriched for beneficial mutations.

%In classic models of population genetics, strongly advantageous mutations undergo rapid selective \emph{sweeps}, i.e., they rise rapidly and fix.
%In rapidly adapting population like influenza viruses, however, the sweep of a beneficial mutation is affected by the fitness of the genetic background it arose on and interference by other competing mutations \citep{strelkowa_clonal_2012}.
%By considering the average behavior of variants that are rising in frequency, we effectively average over backgrounds, and obtain the beneficial on average.
...
%Interference between multiple such advantages mutations will perturb the trajectories \citep{illingworth_components_2012} but we still expect that they rise in frequency on average.
...
The fixation probability of a mutation at frequency $f$, $P_{fix}(f)$, is then estimated by the fraction of those trajectories which terminate at a frequency larger than $95\%$, \emph{i.e.} our fixation threshold.
...
No clear difference is found for the lists by \citet{luksza_predictive_2014, wolf_long_2006}, while positions from \citet{Shih6283} fix less often.
...
In fact, \citet{huddleston_integrating_2020} reports that the rate at which the frequency of a strain changes is often a poor predictor -- consistent with our observations here.
...
Furthermore, selection might cause the rapid rise of a novel variant to macroscopic frequencies (observable in a global sample) but its benefit rapidly ``expires'' because competing variants catch up and/or it mediates immune escape only to a small fraction of the population \citep{lee_mapping_2019}.
...
Different age-groups therefore got exposed to more similar A/H1N1pdm variants than their initial exposure to A/H3N2.
...
\label{sub:data}
	...
	Acknowledgement tables with strain names, accession numbers, and originating and submitting labs are given as two supplementary files. Outlier strains listed at \url{https://github.com/PierreBarrat/FluPredictibility/tree/master/src/config} were removed. \\
...
\label{sub:measuring_the_geographical_spread_of_a_mutation}
	...
	Region used are the ones defined in \texttt{Nextstrain/flu}~\cite{10.1093/bioinformatics/bty407}. Those are {North America}, {South America}, {Europe}, {China}, {Oceania}, {Southeast Asia}, {Japan \& Korea}, {South Asia}, {West Asia}, and {Africa}.
...
\label{sub:supplementary_figures}
	...
	\begin{figure}
		...
		\caption{Fixation probability as a function of frequency for the simulations discussed in the main text, with trajectories stratified according to real fitness values. ``High'' and ``low'' fitness classes are defined with respect to the median value. \textbf{Left:} Simulation with a purely additive landscape that changes every $\Delta t=10$ generations. \textbf{Right:} Simulation with antibodies, with a new antibody every $\Delta t=10$ generations. }
